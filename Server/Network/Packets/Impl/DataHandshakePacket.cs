...
namespace Server.Network.Packets.Impl
{
    public class DataHandshakePacket : Packet
    {
        ...
        public override void Handle(byte[] buffer, NetworkStream clientStream)
        {
            ...
            ConsoleUtils.WriteLine($"{(Program.Settings.HideIp ? "" : $"(IP: {Client.Client.Client.RemoteEndPoint}) ")}Client's requested data has fully sent (IsAuthed: {Client.Authed}, Obfuscation: {Program.Settings.Obfuscation})", "NetworkHandler");
        }
    }
}